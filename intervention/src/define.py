...
class Orientation3D(Task):
    # This task tells the robot: “I want the gripper (or a specific link) to face a certain direction (yaw).
    # Tell me how far off I am from that direction, and how I should move to fix it.”

    # The Orientation3D task helps the robot rotate its hand or tool to face the correct direction. 
    # It checks how far off the current rotation is from the goal, figures out which joints can help fix it, 
    # and tells the controller how to adjust them to reach the desired orientation.
    ...
    def update(self, robot):
        ...
        orien = np.arctan2(k[1, 0], k[0, 0])  # Calculate current orientation
        self.err = self.getDesired() - orien  # Update task error | How far off is the current orientation from the desired one?

class BaseOrientation3D(Task):

# BaseOrientation3D is a task that helps the robot align its gripper by turning the base (wheels) instead of twisting the arm. 
# It checks how far the current orientation is from the target, figures out how much base yaw would help, 
# and updates the control system to rotate accordingly.
...
class Configuration3D(Task):
    # This task tells the robot: “Move the end-effector to this position AND rotate it to this orientation 
    # — all at the same time.”

    # The Configuration3D task controls both the position and the yaw orientation of the gripper together. 
    # It figures out how much the robot is off from the desired configuration, and computes how moving 
    # each joint or the base affects that — so the controller can correct it all in one smooth motion.

    # ----  In the project we used Position3D to move the gripper to the right location, 
    # ----  and does not control orientation of the gripper during pick/place. -----
    # ----  Since our goal is to just reach and grip, regardless of which way the gripper is facing.  ---
    def __init__(self, name, desired, link):
        """
    ...
    def update(self, robot):
        ...
        robot (Manipulator): Reference to the manipulator.
        """
        # First 3 rows: how position is affected by base + joints
        self.J[0:3, :] = robot.getEEJacobian(self.link)[0:3]  # Update position Jacobian
        # Last row: how yaw (orientation) is affected by base + joints
        self.J[-1, :] = robot.getEEJacobian(self.link)[-1].reshape(1, 6)  # Update orientation Jacobian

        # Gets the 4×4 matrix of the gripper's pose
        transf = robot.getEETransform()  # Get end-ef  fector transformation
        # Extracts the position [x, y, z] from the last column
        eep = transf[0:3, 3].reshape(3, 1)  # Get end-effector position
        # Extracts the yaw (rotation around Z) from the rotation matrix
        orien = np.arctan2(transf[1, 0], transf[0, 0])  # Calculate orientation
        # Stack the 3D position and 1D orientation into a 4×1 vector
        self.config = np.vstack([eep, orien])  # Combine position and orientation
        #The result is the error vector for both position and orientation
...
class JointPosition3D(Task):
    # This task controls just one joint angle. It checks how far the joint is from the desired angle, 
    # then computes how to move it back to the target using the robot’s full Jacobian.
